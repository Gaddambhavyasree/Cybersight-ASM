  ...
  return (
    <div className="card">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h3 className="text-lg font-semibold text-slate-100">Top Open Ports</h3>
      </div>
      {topPorts.length === 0 ? (
        ...
        <div className="space-y-3">
          {topPorts.map((item) => (
            <div key={item.port} className="flex items-center gap-3">
              <div className="w-16 text-right font-mono text-slate-300 text-sm">
                {item.port}
              </div>
              <div className="flex-1 h-7 bg-slate-800 rounded-lg overflow-hidden relative">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-medium text-slate-200">
                  {item.count}
                </span>
              </div>
            </div>
