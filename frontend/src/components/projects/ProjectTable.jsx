      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-800/60">
          {projects.map((project) => (
            <tr key={project.id} className="hover:bg-[var(--muted)] transition-colors">
              <td className="py-3.5">
                <p className="font-medium text-[var(--foreground)]">{project.name}</p>
                {project.tags?.length > 0 && (
                  <div className="flex gap-1 mt-1 flex-wrap">
                    {project.tags.slice(0, 3).map((tag) => (
                      <span key={tag} className="text-[10px] bg-[var(--muted)] text-[var(--muted-foreground)] px-1.5 py-0.5 rounded">
                        {tag}
                      </span>
                    ))}
              ...
              <td className="py-3.5 text-[var(--muted-foreground)]">{project.organization || '--'}</td>
              <td className="py-3.5">
                <span className="text-[var(--primary)] font-mono text-xs">{project.target_domain}</span>
              </td>
              <td className="py-3.5">
              ...
              <td className="py-3.5 text-[var(--muted-foreground)] text-xs">{project.created_by_name || '--'}</td>
              <td className="py-3.5 text-[var(--muted-foreground)] text-xs">{formatDate(project.created_at)}</td>
              <td className="py-3.5">
