    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Project Details">
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-slate-200 font-medium">{project.name}</p>
        </div>

        {project.organization && (
          <div>
            <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Organization</p>
            <p className="text-sm text-slate-200">{project.organization}</p>
          </div>
        )}

        <div>
          <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Target Domain</p>
          <p className="text-sm text-sky-400 font-mono">{project.target_domain}</p>
        </div>
          ...
          <div>
            ...
            <div className="flex gap-1.5 flex-wrap">
              {project.tags.map((tag) => (
                <span key={tag} className="text-xs bg-slate-800 text-slate-400 px-2 py-1 rounded-md border border-slate-700/50">
                  {tag}
                </span>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-800">
          <div>
            ...
            <p className="text-sm text-slate-300">{project.created_by_name || '--'}</p>
          </div>
          <div>
            <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Created At</p>
            <p className="text-sm text-slate-300">{formatDate(project.created_at)}</p>
          </div>
        </div>
